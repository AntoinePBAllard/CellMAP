...
%     channelName = strsplit(channelName{1},' ');
    for channel_name = {'measuredHeight'}
        ...
        for conversion_name = conversionName
            ...
            data.(segment_style{segment_number+1}).(channel_name{:}).(conversion_name{:}).multiplier = multiplier;
            data.(segment_style{segment_number+1}).(channel_name{:}).(conversion_name{:}).offset = offset;
        end
    end
end
        
for segment_number = 0:segment_count-1
    for channel_name = channelName
        lcd_info_location = strcmp(segment_header_metadata_split(:,1),strcat('channel.',channel_name{:},'.lcd-info.*'))==1;
        lcd_info = segment_header_metadata_split(lcd_info_location,2);
        ...
        for conversion_name = conversionName
            data.(segment_style{segment_number+1}).(channel_name{:}).(conversion_name{:}).data = nan(str2double(num_points{segment_number+1}));
            index = (row-1)*ilength+col-1;
            channel_file = fullfile(InfoDir(1).folder,'index',num2str(index),'segments',num2str(segment_number),'channels',[channel_name{:} '.dat']);
            fid=fopen(channel_file);
            flag_temp=fread(fid,'integer*4','ieee-be');
            fclose(fid);
            multiplier = data.(segment_style{segment_number+1}).(channel_name{:}).multiplier;
            offset = data.(segment_style{segment_number+1}).(channel_name{:}).offset;
            flag_temp = flag_temp*multiplier+offset;
            multiplier = data.(segment_style{segment_number+1}).(channel_name{:}).(conversion_name{:}).multiplier;
            offset = data.(segment_style{segment_number+1}).(channel_name{:}).(conversion_name{:}).offset;
            data.(segment_style{segment_number+1}).(channel_name{:}).(conversion_name{:}).data = (flag_temp+offset)*multiplier;
        end
    end
end
...
indent = extractheight(end);
height = extractheight*1e6; % in [um]
force = (extractforce-(extractheight*Data_Processed.Baseline_Slope_0x5BN0x2Fm0x5D(index+1)+Data_Processed.Baseline_Offset_0x5BN0x5D(index+1)))*1e9;
end
