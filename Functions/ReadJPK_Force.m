function [height,force] = ReadJPK_Force(folder)
...
height = NaN(length(num_points),num_points(1));
force = NaN(length(num_points),num_points(1));
for k = 0:(length(num_points)-1)
    ...
    for channel_name = {'measuredHeight'}
        ...
        for conversion_name = {'nominal'}%conversionName
            ...
            height(k+1,1:length(data_tmp)) = data_tmp*1e6;
        end
    end
    ...
    for channel_name = {'vDeflection'}
        ...
        for conversion_name = {'force'}%conversionName
            ...
            data.(['segment' num2str(k)]).(channel_name{:}).(conversion_name{:}).data = (flag_temp+offset)*multiplier;
            data_tmp = data.(['segment' num2str(k)]).(channel_name{:}).(conversion_name{:}).data;
            force(k+1,1:length(data_tmp)) = data_tmp*1e9;
